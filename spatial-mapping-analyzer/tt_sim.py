...
class TTSimBackend:
    ...
    def _invoke(self, workdir, library_hash):
        command = [
            ...
            str(self.library),
            "--manifest",
            str(workdir / self.manifest_filename),
            "--result",
            str(workdir / "runner_result.json"),
        ]
        write_json(
            workdir / "invocation.json",
            {
                "argv": command,
                "cwd": str(ROOT),
    ...
    def run(self, architecture, program, mapping, workdir):
        ...
        scope = {
            ...
            "mapping_lowered": False,
            "compute_path": "BRISC RV32IM; no Tensix FPU/SFPU",
            "transfers": "host-mediated PCI/BAR",
        }
        ...
        def failure(code, message, status="error"):
            return Report(
                **identity,
                status=status,
                message=message,
                extensions={**scope, "error_code": code},
            )
        ...
        write_json(workdir / "reference.json", expected)
        ...
        passed = (
            result.get("tensors") == expected
            and result.get("outputs") == {tensor: all_values[tensor] for tensor in program.outputs}
        )
        scope.update(program_dag_executed=True, mapping_lowered=True, execution=result)
        write_json(
            workdir / "correctness.json",
            {
                "passed": passed,
                "comparison": "exact int32, all intermediate tensors and outputs",
            },
        )
        ...
        return Report(
            **identity,
            ...
            metrics=unsupported_metrics(
                "BRISC correctness harness does not measure accelerator performance"
            ),
            message=(
                "DAG matches CPU reference; no performance objective is available"
                if passed
                else "DAG differs from CPU reference"
            ),
            extensions=scope,
        )
